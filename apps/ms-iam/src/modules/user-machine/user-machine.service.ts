...
export class UserMachineService {
  ...
  async findAll() {
    return await this.userMachineModel.find().populate('User').populate('Machine').exec();
  }

  async findOne(id: string) {
    return await this.userMachineModel.findById(id).populate('User').populate('Machine').exec();
  }
